...
class ServerState
{
    ...
    public static function inPreparation()
    {
        $sqlQuery = "SELECT * FROM `serverstate` WHERE EndDate IS NULL AND (`Type`='WINTERPREPARATION' OR `Type`='SUMMERPREPARATION' )";
        return getCommand($sqlQuery) !== false;
    }
...
AND (`Type`='NORECRUTATION' OR`Type`='RECRUTATION1' OR `Type`='RECRUTATION2' OR `Type`='RECRUTATION3')";
  ...
  AND (`Type`!='NORECRUTATION' AND`Type`!='RECRUTATION1' AND `Type`!='RECRUTATION2' AND `Type`!='RECRUTATION3')";
    ...
    {
        $studentsData = getCommand("
SELECT students.*, persons.Name, persons.Surname,persons.userType,persons.Login
FROM `students` INNER JOIN `persons` ON `persons`.`id` = `students`.`personID` WHERE `Term` = 0");
